...
// Mega Drive Dumper Specific Pinout
...
//Mega Drive Dumper Specific VAR
...
static unsigned int slotRegister = 0; 								//for Sega Master System
static unsigned int slotAdr = 0; 									//for Sega Master System
...
static const char *usb_strings[] =
	{
    "Jackobo Le Chocobo - 06/2024",
    "Mega Drive Dumper STM32F4",
    serial_no,
	};
...
static void usbdev_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
    ...
    if (usb_buffer_IN[0] == READ_MD && usb_buffer_IN[4] != 1 )   	//READ Mega Drive Exchange mode (Low Speed)
		{
        dump_running = 0;
        readMd();
        usbd_ep_write_packet(usbd_dev, 0x82,usb_buffer_OUT,64);
    ...
    if (usb_buffer_IN[0] == READ_MD && usb_buffer_IN[4] == 1 )   	//READ Mega Drive Streaming mode (High Speed)
		{
        dump_running = 1;
        SendNextPaquet(usbd_dev,0x82);
		}
	
    if (usb_buffer_IN[0] == READ_MD_SAVE)  						 	//READ Mega Drive Save
		{
        dump_running = 0;
        usbd_ep_write_packet(usbd_dev, 0x82,usb_buffer_OUT,64);
		}
	
	if (usb_buffer_IN[0] == READ_SMS)								//READ Sega Master System
		{
		dump_running = 0;		
		readSMS();
		usbd_ep_write_packet(usbd_dev, 0x82,usb_buffer_OUT,64);	
...
static void gpio_setup(void)
    ...
    //Enable GPIOC clock. Using API functions:
...
void readSMS()
	...
	if(slotAdr > 0x7FFF)
		{
		gpio_set(GPIOB,CE); //CE md == A15 in SMS mode !
		}
...
void EraseFlash()
	{
    //compatible - SST39SF020 , SST39SF040
	setDataOutput();
